...
impl Region {
    ...
    pub fn active_workspace(&self) -> Workspace {
        let mut inner = self.inner.borrow_mut();
        if let Some(workspace) = &inner.active_workspace {
            workspace.clone()
        } else {
            error!("a region should allways have a region, adding one");
            let workspace = Workspace::new(":autogenerated".to_owned());
            inner.workspaces.insert(workspace.clone());
            inner.active_workspace = Some(workspace.clone());
            workspace
        }
    }
